...
const STRINGS_BY_LOCALE: Record<AmcRenderLocale, AmcRenderStrings> = {
  ko: {
    ...
    experimentDesign3: "전환 범위를 확대하기 전에 스폰서, 대안, 지원 기반이 지속 가능한지 검증합니다.",
    reassessmentTriggerDefault:
      "Decision Conditions가 충족되기 전에 핵심 구조 신호가 약해지면 재평가가 필요합니다.",
    reassessmentTriggerSignalInstability:
      "핵심 구조 신호의 일관성이 약해지면, 조건을 다시 정리하기 전에 재평가가 필요합니다.",
    reassessmentTriggerTimingMisalignment:
      "결정의 속도와 실행 준비도가 맞지 않기 시작하면 재평가가 필요합니다.",
    reassessmentTriggerSupportErosion:
      "스폰서, Safety Margin, 조직 안정성 등 전환을 지지하는 기반이 약해지면 재평가가 필요합니다.",
    reassessmentTriggerRiskDeterioration:
      "핵심 구조 리스크가 커지면, 전환 범위를 확대하기 전에 재평가가 필요합니다.",
    ...
    externalCueFallback: "External Validation: 핵심 외부 신호는 아직 부분적으로만 정리되어 있습니다.",
    externalCueWeakEvidence: "External Validation: 외부 신호는 확인되지만, 아직 충분한 검증으로 이어지지는 않았습니다.",
    externalCuePrefix: "External Validation",
    ...
    matrixCueFallback: "Structural Reading: 현재 구조 신호는 전반적으로 엇갈려 있습니다.",
    matrixCuePrefix: "Structural Reading",
